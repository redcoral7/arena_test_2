...
function App() {
  ...
  const shopItems = [
    ...
    { id: 2, name: '개껌', price: '1,500', desc: '훈련용 간식입니다.', detail: '투견들의 스트레스 해소와 치아 건강을 위해 특수 제작된 고단백 개껌입니다. 민트 향이 첨가되어 있습니다.' },
    { id: 3, name: '진단서', price: '10,000', desc: '공식 의료 기록지입니다.', detail: '아레나 지정 병원에서 발행하는 정식 진단서입니다. 사유서 제출 시 증빙 자료로 활용 가능합니다.' }
  ];
              ...
              {shopItems.map((item) => (
                <div key={item.id} onClick={() => setSelectedItem(item)} className="group cursor-pointer bg-[#050505] border border-zinc-900 p-1 hover:border-red-600 transition-all duration-500 shadow-2xl">
                  ...
                  <div className="p-6 bg-black border-t border-zinc-900 group-hover:border-red-900 transition-colors">
                      ...
                      <p className="text-zinc-600 text-[11px] font-medium leading-relaxed italic border-t border-zinc-900/50 pt-2 group-hover:text-zinc-400 transition-colors">{item.desc}</p>
                  </div>
                </div>
              ))}
            </div>
                    ...
                    mails.filter(m => m.sender_code === user.code || m.receiver_code === user.code).map(m => (
                      <div key={m.id} className="bg-black border border-zinc-900/50 p-4 flex justify-between items-center">
                        <div className="flex flex-col">
                          <span className="text-[9px] text-zinc-700 font-black uppercase mb-1">{m.title.includes('[사유서]') ? '사유서' : '건의사항'}</span>
            ...
            <div className="border border-red-900/20 p-12">
              ...
              <div className="space-y-8">
                ...
                <div>
                  ...
                  <input 
                    type="file" 
                    className="w-full bg-black border border-zinc-900 p-4 text-[11px] text-zinc-500 italic file:bg-red-900 file:border-none file:text-white file:px-4 file:py-1 file:mr-4 file:font-black file:uppercase file:cursor-pointer cursor-pointer" 
                ...
                <div className="flex gap-4">
                  <button 
                    ...
                    className="flex-1 bg-red-900 py-5 font-black text-white hover:bg-red-700 uppercase tracking-widest text-sm shadow-xl transition-all active:scale-95 disabled:opacity-50"
                  >
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/98 p-4 backdrop-blur-2xl">
          <div className="bg-black border border-red-900 w-full max-w-2xl p-12 relative animate-in zoom-in-95">
            ...
            <div className="bg-[#050505] border border-zinc-900 p-10 mb-12 border-l-4 border-l-red-900">
              <p className="text-zinc-400 leading-relaxed italic text-lg">{selectedItem.detail}</p>
            </div>
            <div className="flex justify-between items-center border-t border-zinc-900 pt-10">
              <span className="text-4xl font-black text-white italic">{selectedItem.price} <span className="text-sm">PTS</span></span>
