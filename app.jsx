...
/**
 * [SUB COMPONENT] DuelRequestModal: 결투 신청 실시간 팝업
 */
const DuelRequestModal = ({ isOpen, mailData, onAccept, onReject }) => {
  if (!isOpen || !mailData) return null;
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/90 backdrop-blur-md">
      <div className="relative w-[420px] border border-red-900/40 bg-[#0a0000] p-10 shadow-[0_0_50px_rgba(139,0,0,0.4)] animate-in zoom-in-95 duration-300">
        ...
        <div className="mb-12 text-center">
          <p className="text-xl text-gray-300 font-medium leading-relaxed">
            <span className="text-red-500 font-bold underline">[{mailData.sender_name}]</span> 님이<br />
            결투를 신청했습니다.
          </p>
        </div>
...
/**
 * [SUB COMPONENT] StockMarket: 주식 시스템
 */
  ...
  const simulatePrice = async () => {
    ...
    for (const s of currentStocks) {
      ...
      await supabaseClient.from('stocks').update({ 
        ...
        change_rate: ((newPrice - s.current_price) / s.current_price) * 100 
      }).eq('id', s.id);
    }
    refreshData();
  };

  const handleBuy = async (sName, price) => {
    const qty = parseInt(prompt(`${sName}을(를) 몇 주 매수하시겠습니까?`, "1"));
    if (!qty || qty <= 0) return;
    const cost = qty * price;
    if (user.points < cost) return alert("PTS가 부족합니다.");
    const currentPos = myStocks.find(s => s.stock_name === sName);
    if (currentPos) {
      const newQty = currentPos.quantity + qty;
      const newAvg = Math.floor((currentPos.avg_price * currentPos.quantity + cost) / newQty);
      await supabaseClient.from('user_stocks').update({ quantity: newQty, avg_price: newAvg }).eq('id', currentPos.id);
    } else {
      await supabaseClient.from('user_stocks').insert([{ user_code: user.code, stock_name: sName, quantity: qty, avg_price: price }]);
    }
    await supabaseClient.from('users').update({ points: user.points - cost }).eq('code', user.code);
    alert("매수 완료"); fetchUserList(); refreshData();
  };

  const handleSell = async (sName, price) => {
    const pos = myStocks.find(s => s.stock_name === sName);
    if (!pos || pos.quantity <= 0) return alert("보유 주식이 없습니다.");
    const qty = parseInt(prompt(`판매 수량 (보유: ${pos.quantity}주)`, pos.quantity));
    ...
    alert("매도 완료"); fetchUserList(); refreshData();
  };

  if (loading) return <div className="text-center py-20 text-red-900 font-black animate-pulse">CONNECTING TO ARENA EXCHANGE...</div>;
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 animate-in fade-in duration-700">
      <div className="bg-[#050505] border border-zinc-900 p-8 shadow-2xl">
        <h3 className="text-red-600 font-black italic mb-8 uppercase tracking-[0.3em] border-l-2 border-red-900 pl-4">Market Quotes</h3>
            ...
            <div key={s.id} className="p-5 border border-zinc-900 bg-black flex justify-between items-center group hover:border-red-900 transition-all">
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="flex gap-1">
                  <button onClick={() => handleBuy(s.name, s.current_price)} className="bg-red-900/10 border border-red-900 px-4 py-2 text-red-600 text-[10px] font-black hover:bg-red-900 hover:text-white uppercase transition-all">Buy</button>
                  <button onClick={() => handleSell(s.name, s.current_price)} className="bg-blue-900/10 border border-blue-900 px-4 py-2 text-blue-600 text-[10px] font-black hover:bg-blue-900 hover:text-white uppercase transition-all">Sell</button>
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="bg-[#050505] border border-zinc-900 p-8 shadow-2xl">
        ...
        <div className="space-y-3">
          {myStocks.length > 0 ? myStocks.map(ms => {
            ...
            return (
              <div key={ms.id} className="p-5 border border-zinc-900 bg-black flex justify-between items-end">
                <div><div className="text-zinc-400 font-bold mb-1">{ms.stock_name}</div><div className="text-[10px] text-zinc-700 uppercase font-black">Qty: {ms.quantity} / Avg: {ms.avg_price.toLocaleString()}</div></div>
          ...
          }) : <div className="py-20 text-center text-zinc-800 italic text-[10px] uppercase border border-dashed border-zinc-900">No Positions</div>}
        </div>
      </div>
    </div>
  );
...
function App() {
  ...
  const sendMail = async () => {
    ...
    if (isGift && !targetInvItem) return alert('보유한 아이템을 선택해주세요.');
    ...
    try {
      ...
      const { error } = await supabaseClient.from('mails').insert([{ 
        ...
        title: finalTitle, content: finalContent, status: '처리대기'
      }]);

      if (!error) {
        if (isGift) await supabaseClient.from('user_inventory').delete().eq('id', targetInvItem.id);
        alert('전송 완료'); setIsMailFormOpen(false); setMailForm({ category: '건의사항', title: '', targetUser: '', content: '', selectedGiftItem: '' });
        setSelectedFile(null); fetchInventory(); fetchAllMails();
      }
    } catch (e) { alert('전송 오류'); } finally { setIsUploading(false); }
  };
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-400 font-serif overflow-x-hidden">
      <nav className="px-8 py-4 flex justify-between items-center border-b border-red-950/30 bg-black/90 backdrop-blur-xl sticky top-0 z-50">
        <div className="flex items-center gap-12">
          ...
          <div className="flex gap-10 items-center text-[11px] font-black tracking-[0.4em] uppercase">
             ...
             <button onClick={() => setView('shop')} className={view === 'shop' ? 'text-red-600' : 'text-zinc-600'}>[ SHOP ]</button>
             <button onClick={() => setView('stock')} className={view === 'stock' ? 'text-yellow-600' : 'text-zinc-600'}>[ STOCK ]</button>
        ...
        <div className="flex gap-6 items-center">
          {user ? (
            <>
              ...
              <button onClick={() => setIsUserProfileOpen(true)} className="text-red-600 font-black italic border-b border-red-900 uppercase">{user.name} ▾</button>
              <button onClick={handleLogout} className="text-[10px] text-zinc-600 border border-zinc-800 px-3 py-1 font-black">LOGOUT</button>
            </>
          ) : <button onClick={() => setIsLoginOpen(true)} className="text-red-700 font-black text-[11px] border border-red-900 px-8 py-2 italic tracking-widest">LOGIN</button>}
        </div>
      </nav>

      <main className="max-w-7xl mx-auto pt-24 px-8 pb-32">
        {view === 'home' && (
          <div className="flex flex-col items-center justify-center pt-40 text-center animate-in fade-in duration-1000">
              ...
              {shopItems.map(item => (
                <div key={item.id} onClick={() => setSelectedItem(item)} className="group cursor-pointer bg-[#050505] border border-zinc-900 p-1 hover:border-red-600 transition-all">
                  <div className="aspect-[4/3] bg-zinc-950 flex items-center justify-center overflow-hidden">
                    {item.image_url ? <img src={item.image_url} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all" /> : <span className="text-zinc-900 font-black text-4xl italic uppercase">{item.name}</span>}
                  </div>
        ...
        {view === 'stock' && (
          <div className="animate-in slide-in-from-bottom-8 duration-700">
             <div className="flex justify-between items-end mb-16 border-l-4 border-yellow-700 pl-8 py-2">
              <div><h2 className="text-6xl font-black text-white italic tracking-tighter uppercase mb-2">Exchange</h2><p className="text-yellow-700 font-black tracking-[0.5em] text-[10px] uppercase">Live Simulation</p></div>
              <div className="text-right font-black italic text-4xl text-yellow-600">{user?.points.toLocaleString() || '---'} PTS</div>
            </div>
            {user ? <StockMarket user={user} fetchUserList={fetchUserList} /> : <div className="text-center py-20 text-red-600 font-black uppercase tracking-widest italic">Login Required</div>}
          </div>
        )}
      </main>

      {/* Profile Modal - 유저의 인벤토리 및 기록 확인 */}
      {isUserProfileOpen && user && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/98 p-4 backdrop-blur-3xl">
          <div className="bg-[#050505] border-2 border-red-700 w-full max-w-2xl p-10 animate-in zoom-in-95 overflow-y-auto max-h-[90vh] custom-scrollbar">
            ...
            <div className="bg-black border border-zinc-900 p-8 flex justify-between items-center mb-8">
              ...
              <div className="grid grid-cols-2 gap-2">
                {inventory.length > 0 ? inventory.map((inv, idx) => (
                  <div key={idx} className="bg-zinc-950 border border-zinc-900 p-4 flex items-center gap-3">
                    <div className="w-1.5 h-1.5 bg-red-700 rounded-full shadow-[0_0_5px_red]"></div>
      ...
      {isAdminMailOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/98 p-4 backdrop-blur-md">
          <div className="bg-black border-2 border-red-700 w-full max-w-6xl h-[850px] flex flex-col shadow-2xl">
            <div className="p-10 border-b border-red-900/50 flex items-center justify-between bg-black">
              <div className="flex gap-6">
                {['건의사항', '사유서', '선물하기'].map(tab => (
                  <button key={tab} onClick={() => { setActiveTab(tab); setSelectedMail(null); }} className={`px-12 py-4 text-[11px] font-black border uppercase tracking-widest ${activeTab === tab ? 'border-red-600 text-red-600 bg-red-900/10' : 'border-zinc-800 text-zinc-700'}`}>{tab}</button>
                ))}
              </div>
              ...
              {!selectedMail ? (
                <div className="space-y-4">
                  {mails.filter(m => m.title?.includes(`[${activeTab}]`)).map(m => (
                    <div key={m.id} onClick={() => markAsRead(m)} className="p-8 border border-zinc-900 flex justify-between items-center bg-[#050505] hover:border-red-600 cursor-pointer group">
                      <div className="flex items-center gap-12">
                        ...
                        <span className="text-zinc-500 text-xl italic group-hover:text-zinc-300">{m.title}</span>
                      </div>
                ...
                <div className="h-full flex flex-col">
                  ...
                  <h3 className="text-5xl font-black text-white italic uppercase mb-8 border-b border-zinc-900 pb-8">{selectedMail.title}</h3>
                  <div className="bg-[#050505] border border-zinc-900 p-12 min-h-[300px] flex-1">
                    ...
                    {selectedMail.content.includes('---FILE_URL---') && <a href={selectedMail.content.split('---FILE_URL---')[1]} target="_blank" className="text-red-600 underline font-black uppercase tracking-widest text-xs">Download Attachment</a>}
                  </div>
                  {activeTab === '선물하기' && selectedMail.status === '처리대기' && (
                    <div className="mt-8 flex gap-4">
                      <button onClick={() => { supabaseClient.from('mails').update({ status: '처리완료' }).eq('id', selectedMail.id).then(() => { alert('선물 수락됨'); setSelectedMail(null); fetchAllMails(); }); }} className="flex-1 bg-red-900 py-4 font-black text-white hover:bg-red-700">APPROVE</button>
                      <button onClick={() => { supabaseClient.from('mails').update({ status: '거절됨' }).eq('id', selectedMail.id).then(() => { alert('선물 거절됨'); setSelectedMail(null); fetchAllMails(); }); }} className="flex-1 border border-zinc-800 py-4 text-zinc-600 hover:text-white">REJECT</button>
                    </div>
                  )}
                </div>
              )}
      ...
      {/* Mail Form - 유저의 건의/결투/선물 전송 모달 */}
      {isMailFormOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/98 p-4 backdrop-blur-md">
          <div className="bg-[#050505] border border-red-900/40 w-full max-w-xl p-12 shadow-2xl">
            ...
            <div className="space-y-8">
              <div>
                <label className="text-[9px] text-zinc-700 font-black uppercase mb-3 block tracking-widest">Transmission Category</label>
                <select className="w-full bg-black border border-zinc-900 p-5 text-zinc-400 font-black text-xs outline-none focus:border-red-900" value={mailForm.category} onChange={(e) => setMailForm({...mailForm, category: e.target.value})}>
              ...
              {mailForm.category === '선물하기' && (
                <div>
                  <label className="text-[9px] text-red-700 font-black uppercase mb-3 block">Select Your Item</label>
              ...
              {mailForm.category === '건의사항' ? (
                ...
                <div>
                  <label className="text-[9px] text-zinc-700 font-black uppercase mb-3 block">Recipient Name(Code)</label>
                  <input type="text" className="w-full bg-black border border-zinc-900 p-5 text-white italic outline-none focus:border-red-900" list="userList" placeholder="이름 또는 코드를 입력..." onChange={(e) => setMailForm({...mailForm, targetUser: e.target.value})} />
                  <datalist id="userList">{allUsers.filter(u => u.code !== user.code).map(u => <option key={u.code} value={`${u.name}(${u.code})`} />)}</datalist>
                </div>
              )}
              <div><label className="text-[9px] text-zinc-700 font-black uppercase mb-3 block">Message Body</label><textarea rows="5" className="w-full bg-black border border-zinc-900 p-5 text-white italic outline-none focus:border-red-900" placeholder="내용을 입력하세요..." onChange={(e) => setMailForm({...mailForm, content: e.target.value})}></textarea></div>
              <div><label className="text-[9px] text-red-600 font-black uppercase mb-3 block">File Attachment</label><input type="file" className="w-full bg-black border border-zinc-900 p-4 text-[11px] text-zinc-500 cursor-pointer" onChange={(e) => setSelectedFile(e.target.files[0])} /></div>
              <div className="flex gap-4">
                <button onClick={sendMail} disabled={isUploading} className="flex-1 bg-red-900 py-5 font-black text-white hover:bg-red-700 uppercase tracking-widest text-sm">{isUploading ? 'Sending...' : 'Transmit'}</button>
      ...
      {/* Duel Request Modal Popup */}
      <DuelRequestModal isOpen={isDuelModalOpen} mailData={pendingDuel} onAccept={(id) => { supabaseClient.from('mails').update({ status: '서명완료' }).eq('id', id).then(() => { setIsDuelModalOpen(false); alert('결투 수락됨'); fetchAllMails(); }); }} onReject={(id) => { supabaseClient.from('mails').update({ status: '거절' }).eq('id', id).then(() => { setIsDuelModalOpen(false); alert('결투 거절됨'); fetchAllMails(); }); }} />
      ...
      {isLoginOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/98 p-4 backdrop-blur-3xl animate-in fade-in">
          <div className="w-96 p-16 border border-red-950 bg-black text-center animate-in slide-in-from-bottom-8">
            <h1 className="text-6xl font-black text-red-600 italic mb-16 uppercase animate-pulse tracking-tighter">Arena</h1>
      ...
      {isUserMgmtOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 p-4 backdrop-blur-md">
          <div className="bg-[#050505] border-2 border-red-700 w-full max-w-5xl h-[80vh] flex flex-col shadow-2xl">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-2 custom-scrollbar">
              {allUsers.filter(u => u.name.includes(searchTerm)).map(u => (
                <div key={u.id} className="flex items-center px-6 py-4 bg-black border border-zinc-900 hover:border-red-600 group">
      ...
      {/* FAB - Mail 전송 버튼 (일반 유저용) */}
      {user && !user.is_admin && (
        <button onClick={() => setIsMailFormOpen(true)} className="fixed bottom-12 right-12 w-16 h-16 bg-red-950 rounded-full flex items-center justify-center text-3xl shadow-[0_0_30px_rgba(153,27,27,0.4)] border border-red-700 hover:scale-110 active:scale-90 transition-all z-40 group animate-bounce">
